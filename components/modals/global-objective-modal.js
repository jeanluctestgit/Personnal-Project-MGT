...
export class GlobalObjectiveModal {
  constructor(stateManager, objective = null, projectId = null, taskId = null) {
      ...
      <div class="modal-overlay">
        <div class="modal-content">
          <div class="modal-header">
            <h2>${this.isEdit ? 'Editer l\'Objectif Global' : 'Nouvel Objectif Global'}</h2>
  ...
  async save() {
    ...
    if (this.isEdit) {
      objectiveData.project_id = this.objective.project_id || null;
      objectiveData.task_id = this.objective.task_id || null;
    } else {
      if (this.projectId) {
        objectiveData.project_id = this.projectId;
        objectiveData.task_id = null;
      } else if (this.taskId) {
        objectiveData.task_id = this.taskId;
        objectiveData.project_id = null;
      }
    }

    try {
      if (this.isEdit) {
        const updated = await apiService.updateGlobalObjective(this.objective.id, objectiveData);
        this.stateManager.updateGlobalObjective(updated);
      } else {
        const created = await apiService.createGlobalObjective(objectiveData);
        this.stateManager.addGlobalObjective(created);
      }
