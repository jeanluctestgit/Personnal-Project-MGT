class StateManager {
  ...
  updateChecklistItems(checklistItems) {
    ...
    this.emit('checklistUpdated', checklistItems);
  }
  ...
  addAttachment(attachment) {
    const attachments = [attachment, ...this.globalState.attachments];
    this.updateAttachments(attachments);
  }

  deleteAttachment(attachmentId) {
    const attachments = this.globalState.attachments.filter(a => a.id !== attachmentId);
    this.updateAttachments(attachments);
  }

  addChecklistItem(item) {
    const checklistItems = [...this.globalState.checklistItems, item];
    this.updateChecklistItems(checklistItems);
  }

  updateChecklistItem(updatedItem) {
    const checklistItems = this.globalState.checklistItems.map(item =>
  ...
  deleteChecklistItem(itemId) {
    const checklistItems = this.globalState.checklistItems.filter(item => item.id !== itemId);
    this.updateChecklistItems(checklistItems);
  }
}
